'use client';
import { approve } from './contract';
import GnosisParty from './GnosisParty';
import TokenUSDCAmount from './TokenUSDCAmount';
...
const sendTopUpRequest = async ({ wallet, amountUsdc }: {
  ...
  const approvalReceipt = await approve(
    wallet,
    (process.env.NEXT_PUBLIC_TOP_UP_GAS_SPONSOR || '') as `0x${string}`,
  ...
  await fetch(
    process.env.NEXT_PUBLIC_GNOSIS_TOP_UP_API || "",
    {
      method: "POST",
      headers: {
